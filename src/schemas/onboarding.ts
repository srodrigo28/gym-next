...
export const baseOnboardingSchema = z.object({
  ...
  workRoutine: z.enum(["mostly_sitting", "mostly_standing", "moderate_physical", "intense_physical", "varies"], { message: requiredMessage }),
  profession: z.string().trim().optional().refine((value) => !value || value.length >= 2, "Informe pelo menos 2 caracteres."),
  ...
  previousTrainingTime: z.enum(["less_than_3_months", "3_to_6_months", "6_to_12_months", "1_to_2_years", "more_than_2_years"]).optional(),
  ...
  beerConsumption: z.enum(["no", "rarely", "weekly_1_2", "weekly_3_plus"], { message: requiredMessage }),
  ...
  relationshipStatus: z.enum(["single", "married", "serious_relationship", "other"], { message: requiredMessage }),
  hasRelationshipDifficulty: z.enum(["no", "sometimes", "often"], { message: requiredMessage }),
  moodPattern: z.enum(["stable", "motivated", "anxious", "irritated", "discouraged", "varies"], { message: requiredMessage }),
  trainsProfessionally: z.enum(["yes", "moderate", "no"], { message: requiredMessage }),
  professionalTrainingPurpose: z.enum(["competition", "bodybuilding", "professional_performance", "physical_test", "other"]).optional(),
});
...
export const onboardingSchema = baseOnboardingSchema.superRefine((data, ctx) => {
  if (data.gymExperience !== "never" && !data.previousTrainingTime) {
    ctx.addIssue({ code: "custom", message: requiredMessage, path: ["previousTrainingTime"] });
  }

  if (data.drinksSoda === "yes" && !data.sodaFrequency) {
    ctx.addIssue({ code: "custom", message: requiredMessage, path: ["sodaFrequency"] });
  }

  if (data.drinksSoda === "yes" && !data.sodaAmount) {
    ctx.addIssue({ code: "custom", message: requiredMessage, path: ["sodaAmount"] });
  }

  if (data.trainsProfessionally === "yes" && !data.professionalTrainingPurpose) {
    ctx.addIssue({ code: "custom", message: requiredMessage, path: ["professionalTrainingPurpose"] });
  }
});
